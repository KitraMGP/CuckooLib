...
public class MessageGuiClient implements IMessage {
    private PacketBuffer data;
    private int window;

    public MessageGuiClient() {

    }

    public MessageGuiClient(PacketBuffer data, int window) {
        this.data = data;
        this.window = window;
    }
    ...
    public static class Handler implements IMessageHandler<MessageGuiClient, IMessage> {
        @Override
        public IMessage onMessage(MessageGuiClient msg, MessageContext ctx) {
            CuckooLib.proxy.getThreadListener(ctx).addScheduledTask(() -> {
                ...
                if (player != null) {
                    ...
                    if (container instanceof ModularContainer) {
                        ...
                        widget.receiveMessageFromClient(msg.data);
                    }
                }
            });
            return null;
